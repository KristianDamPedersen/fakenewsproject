...
We tokenized the text by first splitting it into
...
After tokenizing the content column, we were then able to train a TF-IDF (Term Frequency - Inverse Document Frequency)
...
There were a few reasons for the differences. \textbf{Logistic regression}: For our simple model we initially set the TF-IDF and SVD parameters quite low. We
    ...
    dimensional vector, that then got reduced to 384 dimensions. \textbf{Alternative DNN} : Lastly chose to omit the
