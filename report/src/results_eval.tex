...
These different metrics can be difficult to parse when evaluating on unbalanced datasets. This is where visual aids can come in handy. Confusion matrixes and ROC curves effectively show how horribly bad our models were, at predicting out of domain. In the confusion matrices, we see all guesses arranged in 'columns', which represents a biased classifier (figure \ref{fig:conf_mat}). The ROC curves (figure \ref{fig:roc}) likewise show (almost) zero predictive power for all models.
